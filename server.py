...
#Коефициенты для преобразования шагов в мм
...
def sign(a): #возвращает знак числа
	if a>0:
		return 1;
	elif a<0:
		return -1;
...
def Vertical_Motor_Step(stepper3, dz, speed):
	...
	for i in range(0, int(dz)):
		#stepper3.move(dirZ,1000000.0/speed);
		stepper3.step(dirZ);
	return 0;
...
#   direction is reflected in the polarity of [dt] or [dy]
	dirX=sign(dx);  #get dirction from the polarity of argument [step]
	dirY=sign(dy);
	dirE=-sign(de); #direction backwards for stepper
